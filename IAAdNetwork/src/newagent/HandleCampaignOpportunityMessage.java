...
public class HandleCampaignOpportunityMessage {
	...
	public void run(SampleAdNetworkModified adNetwork, CampaignOpportunityMessage com) {
		...
		/*
		 * Check today campaign that conflict with Winning campaign(s) use
		 * isConflictWithWinningCampaign to check conflicting status
		 */
		...
		for (Iterator<CampaignLogReport> it = adNetwork.getWinCampaigns().iterator(); it.hasNext();) {
			...
			for (Iterator<MarketSegment> it1 = clr.getTargetSegment().iterator(); it1.hasNext();) {
				...
				for (Iterator<MarketSegment> it2 = adNetwork.getPendingCampaign().targetSegment.iterator(); it2
						.hasNext();) {
		...
		/*
		 * Check today campaign that conflict with Other campaign(s) use
		 * isConflictWithOtherCampaign to check conflicting status
		 */
		...
		/*
		 * Check today campaign that conflict with our winning campaign's
		 * duration day use isDaysConflictWithWinningCampaign to check
		 * conflicting status
		 */
		boolean isDaysConflictWithWinningCampaign = false;
		for (Iterator<CampaignLogReport> it = adNetwork.getWinCampaigns().iterator(); it.hasNext();) {
			CampaignLogReport clr = it.next();
			if ((adNetwork.getPendingCampaign().dayEnd == clr.getDayStart()
					&& adNetwork.getPendingCampaign().dayStart <= clr.getDayStart())
					|| (adNetwork.getPendingCampaign().dayStart <= clr.getDayStart()
							&& adNetwork.getPendingCampaign().dayEnd >= clr.getDayStart())
					|| (adNetwork.getPendingCampaign().dayStart >= clr.getDayStart()
							&& adNetwork.getPendingCampaign().dayEnd <= clr.getDayEnd())
					|| (adNetwork.getPendingCampaign().dayStart == clr.getDayEnd()
							&& adNetwork.getPendingCampaign().dayEnd >= clr.getDayEnd())) {
		...
		if (isDaysConflictWithWinningCampaign) {
			...
			System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
			multiplier *= 1 + ((adNetwork.getCurrCampaign().dayEnd - adNetwork.getPendingCampaign().dayStart)
					/ (adNetwork.getPendingCampaign().dayEnd - adNetwork.getCurrCampaign().dayStart));
		}
		...
		/*
		 * During the first five days we do not want to take on a campaign in
		 * order to not compete with every opponent for impressions
		 */
		long cmpBidMillis = random.nextInt((int) cmpimps);
		// cmpBidMillis = (long) (new
		// PredictCampaignCost(adNetwork).predictMultidayPriceIndex(adNetwork) *
		// 1000)*multiplier;
		...
		/*
		 * Example of how to use UserPopulationProbabilities System.out.println(
		 * "probability being computed"); UserPopulationProbabilities
		 * userPopulationProbabilities = new UserPopulationProbabilities(); int
		 * chance = userPopulationProbabilities.getProbability(adNetwork.
		 * getPendingCampaign().targetSegment); System.out.println(
		 * "total probability is " + chance);
		 */
		/*
		 * Note: Campaign bid is in millis
		 */

		AdNetBidMessage bids = new AdNetBidMessage(adNetwork.getUcsBid(), adNetwork.getPendingCampaign().id,
				cmpBidMillis);
